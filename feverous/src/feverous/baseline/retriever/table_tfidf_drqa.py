...
def tf_idf_sim(claim, lines, max_tabs, ngram, hash_size, tokenizer, num_workers, freqs=None):
    tfidf = OnlineTfidfDocRanker(
        [line["sentence"] for line in lines], ngram, hash_size, tokenizer, num_workers, freqs=freqs
    )
    line_ids, scores = tfidf.closest_docs(claim, max_tabs)
    ret_lines = []
...
def tf_idf_claim(line, db, max_page, max_tabs, ngram, hash_size, tokenizer, num_workers, doc_freqs):
    if "predicted_pages" in line:
        ...
        for page in pages:
            ...
            tables = [
                current_page.get_title_content() + "; " + "; ".join(str(c) for c in context[i]) + ". " + str(tab)
        ...
        scores = tf_idf_sim(line["claim"], lines, max_tabs, ngram, hash_size, tokenizer, num_workers, freqs=doc_freqs)
...
def table_tfidf_retrieval(
    ...
    with open("{0}/{1}.pages.p{2}.jsonl".format(data_path, split, max_page), "r") as f, open(
        "{0}/{1}.tables.{4}.p{2}.t{3}.jsonl".format(
            data_path,
            split,
            max_page,
            max_tabs,
        ...
        for line in tqdm(lines):
            line = tf_idf_claim(line, db, max_page, max_tabs, ngram, hash_size, tokenizer, num_workers, doc_freqs)
